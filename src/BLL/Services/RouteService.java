...
import BLL.Interfaces.IRouteService;
import BLL.Interfaces.ILocationService;
import BLL.Services.RoutesCostStrategy.CargoCostRouteStrategy;
import BLL.Services.RoutesCostStrategy.PassengerPlaneRouteStrategy;
import BLL.Services.RoutesCostStrategy.PrivatePlaneRouteStrategy;
...
public class RouteService implements IRouteService {
    ...
    public boolean addRoute(Route route) {
        ...
        route.setCost(this.calculateRouteCost(route));
...
//            }
    ...
    private double calculateRouteCost(Route route){
        RouteCostContext routeCostContext = null;
        switch (route.getPlane().getClass().getSimpleName()) {
            case "PassengerPlane" -> routeCostContext = new RouteCostContext(new PassengerPlaneRouteStrategy());
            case "CargoPlane" -> routeCostContext = new RouteCostContext(new CargoCostRouteStrategy());
            case "PrivatePlane" -> routeCostContext = new RouteCostContext(new PrivatePlaneRouteStrategy());
        }
        if(routeCostContext == null){
            throw  new IllegalArgumentException("Plane is null or invalid type");
        }
        return routeCostContext.executeStrategy(route);
    }
